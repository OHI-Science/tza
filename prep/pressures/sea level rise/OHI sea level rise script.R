...
setwd(here::here('prep/pressures/sea level rise/'))
...
f <- list.files('D:/git/zrasters/sea level rise/',pattern=".tif")     #lists all files in target folder with .tif in file name

for (i in 1:length(f)){

  myraster <- raster(paste('D:/git/zrasters/sea level rise/',f[i],sep=""))
  ...
  write.csv(df, file = paste('D:/git/tza/prep/pressures/sea level rise/Extracted_regional_value _csv//',gsub("\\..*","",f[i]),'.csv',sep=""))   #removes the .tif from file name and replaces with .csv
...
write.csv(regional_values,"D:/git/tza/prep/pressures/sea level rise/Extracted_regional_value _csv/tanzania_all_values_per_region_all_years.csv",row.names = F)
...
write.csv(yearly_scores2,"/data layers/slr_12nm_tan2018.csv",row.names = F)
