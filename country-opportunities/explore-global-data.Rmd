---
title: "Explore OHI Global Data & Data Gaps for Tanzania"
output:
  html_document:
    toc: true
    toc_float: true
...
To explore the OHI Global data for Tanzania, focusing on where data are estimated (gapfilled) instead of available in global data sets for Tanzania.

----

## Gapfilling in Tanzania
...
- **catch by species**: only what is fished within Tanzania's EEZ
- **years**: at least 10 years of data (preferably 20)


We can look at these data in several ways. Let's start by looking on SAUP's website at an interactive visualization of fish caught in Tanzania's exclusive economic zone (EEZ). Here's how: 

1. Navigate to [seaaroundus.org](http://www.seaaroundus.org)
1. Click "Tools & Data"
1. Search "Tanzania"

We can also look at the mapped locations of fishing effort by Tanzania (remember that OHI only includes catch caught withing Tanzania's EEZ. 

1. Click "Mapped Data"

<br>

Let's look through the species listed and see if there are any included that don't represent Tanzania's fisheries within the EEZ. 


#### Mean catch 

The mean catch layer ([fis_meancatch](https://github.com/OHI-Science/tza/blob/master/global_explore/data/fis_meancatch_lookup.csv)) is in some ways a historic list because it includes species that may not be caught any more (i.e. was fished in the past and stopped for some reason). It is calculated as the average (mean) of each species' catch through time, so if it was a big catch in the past but now is 0, it will still average (it keeps all species in the dataset).
...
There are `r fis_meancatch_lookup %>% nrow()` species in the [fis_meancatch](https://github.com/OHI-Science/tza/blob/master/global_explore/data/fis_meancatch_lookup.csv) layer for Tanzania.
...
These are the species stocks that we have data for [fis_b_bmsy](https://github.com/OHI-Science/tza/blob/master/global_explore/data/fis_b_bmsy_lookup.csv):
...
There are only `r fis_b_bmsy_lookup %>% nrow()` species in the [fis_b_bmsy](https://github.com/OHI-Science/tza/blob/master/global_explore/data/fis_b_bmsy_lookup.csv) layer for Tanzania.
...
Let's look at the list of species that are represented in the MAR model as tonnes of harvest: (the [mar_harvest_tonnes](https://github.com/OHI-Science/tza/blob/master/eez/layers/mar_harvest_tonnes.csv) layer). 
...
- are there other species that should be represented in Tanzania?
...
Let's have a look at the data included for Tanzania:
...
Which Natural Products are included for Tanzania?
...
- is this a goal that represents Tanzania well?
...
rgn_spp_gl <- rgn_spp_gl_csv %>%
  ...
  left_join(ico_global_list %>%
            ...
            by = "sciname") %>%
  select(sciname, comname, iucn_sid, cat_code) # n_spp_rgn; this = n_rows: 1467 for Tanzania
